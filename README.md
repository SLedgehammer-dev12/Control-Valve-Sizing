...
- **3.3.0 (current):** İleri mühendislik ve güvenlik paketi — 52 endüstriyel senaryo doğrulama paketi (%100 PASS), çoklu çalışma noktası (Min/Norm/Max) zarf boyutlandırması, Joule-Thomson genleşmesi & Towler-Mokhatab hidrat dengesi, ISO 15848-1 / API 641 kaçak emisyon salmastra seçimi (PTFE chevron, kalıplanmış saf grafit, metal körüklü), API RP 14E erozyonel boru hızı sınırı & API 520 fail-open aşırı basınç tahliye kapasitesi, ISA Form 20 (ISA-S20.50) teknik veri sayfası üreticisi, IEC 60534-8 Whisper Trim / akustik ceket / difüzör iyileştirmesi ve AIV (>110 dBA) uyarısı, kriyojenik (-196 °C) / kanatlı / yüksek sıcaklık boyunları, NACE MR0175 / API 941 Nelson / Duplex 2205 / Monel gövde metalurjisi — 428 test, %91 coverage, ruff + mypy temiz
- **3.2.0:** Hesaplama doğrulama paketi — 12 bağımsız senaryo (sıvı kritik altı/choked/flashing/hız, IEC yayınlı CO₂, analitik choked hava, H₂-NG %20, kızgın/doygunluğa yakın buhar, SI↔US birim, %15 marj, Fisher katalog) bağımsız referanslarla < %0.05 sapma, 12/12 PASS — 328 test, ruff + mypy temiz
